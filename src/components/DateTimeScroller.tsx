export function updateDateTimeScroller(date) {
  ...
  const determineDaySuffix =
    originialNumericSuffixes[
      day % 10 <= 3 && (day % 100) - (day % 10) !== 10 ? day % 10 : 0
    ];

  const formattedDate = `${date.toLocaleDateString(undefined, {
    weekday: "long",
  })} || ${month} ${day}${determineDaySuffix},${year}`;

  return formattedDate;
}

function updateDateTime() {
  const currentDateTimeElement = document.getElementById(
    "currentDateTimeScroller"
  );
